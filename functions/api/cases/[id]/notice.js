...
export async function onRequestPut({ request, env, params }) {
  ...
  if (!f || typeof f !== "object" || typeof f.arrayBuffer !== "function" || f.size === 0) {
    return json({ error: "案内のPDFファイルを選んでください" }, 400);
  }
  if (!env.FILES) return json({ error: "ファイルのアップロード（R2）はまだ使えません" }, 400);
  const ext = NOTICE_MIMES[f.type];
  if (!ext) return json({ error: "期日案内は PDF のみ登録できます" }, 400);
  ...
  await env.DB.prepare(
    `UPDATE cases SET notice_r2_key=?, notice_file_name=?, notice_file_size=?, updated_by=?, updated_at=? WHERE id=?`
  ).bind(key, file.name, file.size, id.email, new Date().toISOString(), cid).run();
...
export async function onRequestDelete({ request, env, params }) {
  ...
  await env.DB.prepare(
    `UPDATE cases SET notice_r2_key=NULL, notice_file_name=NULL, notice_file_size=NULL, updated_by=?, updated_at=? WHERE id=?`
